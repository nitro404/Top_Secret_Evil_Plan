	...
	public static void handlePose(Position position, int angle) {
		...
		if(isStarted() && isIdentified()) {
			...
			if(robotSystem.getActiveRobot().hasActiveBlock()) {
				int x = (int) (robotSystem.getActiveRobot().getActualPosition().getX() - (Math.sin(robotSystem.getActiveRobot().getActualPosition().getAngleRadians()) * (Robot.DISTANCE_TO_FRONT + (Block.SIZE / 2))));
				int y = (int) (robotSystem.getActiveRobot().getActualPosition().getY() - (Math.cos(robotSystem.getActiveRobot().getActualPosition().getAngleRadians()) * (Robot.DISTANCE_TO_FRONT + (Block.SIZE / 2))));
